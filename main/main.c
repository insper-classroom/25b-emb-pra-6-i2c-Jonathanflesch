...
void mpu6050_task(void *p) {
    ...
    while(1) {
      ...
      if(euler.angle.pitch < 20.0 && euler.angle.pitch >-20.0){
        ...
        envio.axis = 1;
        envio.val = (int)euler.angle.pitch;
        xQueueSend(xQueuePos, &envio,0);
      }
      ...
      if(euler.angle.roll < 20.0 && euler.angle.roll >-20.0){
        ...
        envio.axis = 0;
        envio.val = (int)euler.angle.roll;
        xQueueSend(xQueuePos, &envio,0);
      }

      int acc = sqrt((acceleration[0]*acceleration[0])+(acceleration[1]*acceleration[1])+(acceleration[2]*acceleration[2]))-15000;
...
void uart_task(void *p){
    ...
    while(1){
        if(xQueueReceive(xQueuePos, &recebido, pdMS_TO_TICKS(200))){


            uart_putc_raw(uart0, recebido.axis);
            uart_putc_raw(uart0,recebido.val);
            uart_putc_raw(uart0, recebido.val >> 8);
...
int main() {
    ...
    xQueuePos = xQueueCreate(3, sizeof(dados));
